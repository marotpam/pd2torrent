...
def get_show_country(show_summary_url_suffix):
    ...
    summary_data_divs = html_show_summary.find(
        class_='furtherinfo').find_all('li')
...
def get_shows_countries(shows_summaries):
    ...
    for show_summary_url_suffix in shows_summaries:
        shows_countries[show_summary_url_suffix] = get_show_country(
            show_summary_url_suffix)
...
def find_missing_shows_countries():
    ...
    missing_shows = [show for show in all_shows_summaries.values(
    ) if show not in stored_shows.keys()]
...
def get_shows_from(country_name):
    ...
    return [show for show in shows_countries.keys(
    ) if shows_countries[show] == country_name]
