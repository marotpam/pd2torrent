...
def get_episodes_aired_on_date(parsed_html, date):
    episodes = {}
    today_block = parsed_html.find(
        id="d_" + str(date.day) + "_" + str(date.month) + "_" + str(date.year))
...
def get_episode_info_from_link(episode_link):
    ...
    episode_info = {
        'title': str(title), 'season': str(
            matches[0]), 'episode': str(
            matches[1]), 'summary': summary}
    return episode_info
...
def get_episodes_aired_on_dates(username, password, dates):
    ...
    for i, current_date in enumerate(dates):
        ...
        if i == 0:
            ...
            monthly_calendar_html_previous = get_month_html(
                username, password, previous_date)
            monthly_calendar_html_current = get_month_html(
        ...
        today_episodes = get_episodes_aired_on_date(
            monthly_calendar_html_current, current_date)
        yesterday_episodes = get_episodes_aired_on_date(
            monthly_calendar_html_previous, previous_date)

        episodes_aired_info += get_downloadable_episodes(
            today_episodes, yesterday_episodes)
